...
export const getStudyMaterials = async (req, res, next) => {
  try {
    const materials = await query(`SELECT * FROM study_materials ORDER BY id DESC`);
...
export const createStudyMaterial = async (req, res, next) => {
  try {
    const userId = toInt(req.user?.userId || req.user?.id);
    if (!userId) {
    ...
    const result = await query(
      `INSERT INTO study_materials (uploaded_by, title, description, subject, batch, type, file_url, link)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
      [userId, title, description || null, subject || 'General', batchLabel, type || 'PDF', fileUrl || null, link || null]
    );

    const created = await query(`SELECT * FROM study_materials WHERE id = ?`, [result.insertId]);
    return sendSuccess(res, 'Study material uploaded successfully', (created || [])[0] || null, 201);
  } catch (error) {
    next(error);
  }
};
