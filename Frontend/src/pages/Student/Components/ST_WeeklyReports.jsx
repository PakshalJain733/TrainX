    ...
    <div className="weekly-reports-page stack-6">
      ...
      <div className="weekly-status-banner">
        ...
        <p className="weekly-status-desc">
          Automated governance & mentor evaluation portal active.
        </p>
      ...
      <div className="weekly-reports-list">
        {loading ? (
          ...
          reportsData.map((report) => {
            ...
            const overallScore = report.overall_score !== undefined ? `${report.overall_score}%` : (report.score || "0%");
            const attendance = report.attendance_score !== undefined ? report.attendance_score : (report.attendance || 0);
            const quiz = report.quiz_score !== undefined ? report.quiz_score : (report.quiz || 0);
            const coding = report.coding_score !== undefined ? report.coding_score : (report.coding || 0);
            const interview = report.interview_score !== undefined ? report.interview_score : (report.interview || 0);
            const milestones = report.milestones_summary || report.milestones || "0 completed";
            const skillGaps = report.weak_areas || report.skillGaps || [];
